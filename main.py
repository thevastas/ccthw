...
def find_maximum_distance(
    ...
    max_distance = 0
    for city in list(range(number_of_cities)):
        nearest_station = number_of_cities
        for station in cities_with_train_station:
            if abs(city-station) < nearest_station:
                nearest_station = abs(city-station)
        if nearest_station > max_distance:
